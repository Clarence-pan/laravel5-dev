...
RUN DEBIAN_FRONTEND="noninteractive" apt-get install -y --allow-unauthenticated zip php7.1-dom
RUN useradd www && chown -R www:www $APP_ROOT
RUN mkdir -p /var/www && chown -R www:www /var/www
RUN HOME=/var/www USER=www su -p -s /bin/sh -c "composer config -g repo.packagist composer https://packagist.phpcomposer.com && composer create-project laravel/laravel $APP_ROOT --prefer-dist" www
RUN apt-get clean 
